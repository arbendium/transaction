 ...
 * @param {import('./types.js').KeySelector} start
 * @param {import('./types.js').KeySelector} end
 * @param {import('./backend.js').default} backend
 * @returns {Promise<[Buffer, Buffer][]>}
 */
async function getRange(start, end, backend) {
	const result = await backend.getRange(
		start.key,
		start.inclusive,
		start.offset,
		end.key,
...
export default class Transaction {
	...
	async getKey(selector, isSnapshot) {
		...
		const result = typeof resolvedKeySelector === 'number'
			? resolvedKeySelector === -1
				? emptyBuffer
				: this.index.ranges[resolvedKeySelector][0]
	...
	async getRange(
		...
		const queryRange = resolveRangeKeySelectors(start, end, this.index);
